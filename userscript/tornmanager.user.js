...
Stack: ${e.stack}` : ""}`
  ...
  class ChatsSection {
    ...
    openManage(room) {
      ...
      this.client.roomMembers(room.id).then((members) => this.renderManageMembers(room, list, members)).catch((err) => {
      ...
      for (const member of members) {
        ...
        if (!member.host) {
          ...
          action.onclick = () => {
            ...
            call.then(() => this.openManage(room)).catch((err) => {
  ...
  class ChatDock {
    ...
    updateIntegratedButtons() {
      ...
      if (!anchor) return;
      const bar = (_a = anchor.parentElement) == null ? void 0 : _a.parentElement;
      if (!bar) return;
