...
`;
  ...
  class ChatsSection {
    ...
    renderRoom(room) {
      ...
      const members = `${room.member_count} member${room.member_count === 1 ? "" : "s"}`;
      const row = room.host ? this.roomRow(room.name, members, { chip: "Host", chipClass: "tm-chats-room-chip--host" }) : this.roomRow(room.name, members);
