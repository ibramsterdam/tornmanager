...
Stack: ${e.stack}` : ""}`
  ...
  class ChatBox {
    ...
    async openImage(message) {
      ...
      overlay2.className = "tm-lightbox";
      overlay2.style.zIndex = 2147483647;
      const spinner = document.createElement("div");
      spinner.className = "tm-lightbox-spinner";
      overlay2.appendChild(spinner);
      document.body.appendChild(overlay2);
      requestAnimationFrame(() => overlay2.classList.add("tm-lightbox--open"));
      let closed = false;
      const close = () => {
        if (closed) return;
        closed = true;
        overlay2.classList.remove("tm-lightbox--open");
        document.removeEventListener("keydown", onKey);
        setTimeout(() => overlay2.remove(), LIGHTBOX_ANIM_MS);
      };
      const onKey = (e) => {
        if (e.key === "Escape") close();
      };
      overlay2.addEventListener("click", (e) => {
        if (e.target === overlay2 || e.target === spinner) close();
      });
