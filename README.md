...
Torn's rate limit is 100 calls/min per player, not per key. The Keys screen therefore validates every added key (`/user/basic` + `/key/info`) and rejects keys whose owner is already in the pool. More keys from different players means proportionally faster sweeps. Public access keys are sufficient. Every call carries `comment=Recruiter` so key owners can audit usage in their Torn key log.
